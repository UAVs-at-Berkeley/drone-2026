...
su - "$SIM_USER" -c "vncserver :0 -geometry ${VNC_GEOMETRY} -SecurityTypes None -localhost yes"

if su - "$SIM_USER" -c "tmux has-session -t ${SITL_SESSION}" >/dev/null 2>&1; then
  su - "$SIM_USER" -c "tmux kill-session -t ${SITL_SESSION}" || true
fi
su - "$SIM_USER" -c "tmux new-session -d -s ${SITL_SESSION} 'cd /PX4-Autopilot && make px4_sitl ${SITL_MAKE_TARGET}'"
